...
```bash
wget https://robotoperatingsystemscourse.github.io/assets/projects/roscourse-p2-stencil.tar.gz
tar -xf roscourse-p2-stencil.tar.gz
rm roscourse-p2-stencil.tar.gz
```
...
## 1.1: TCP Client and Server Interfaces
### TODO
...
This componenet of Project 2 is meant to simulate the request-response model. There are two programs that you must implement. 
...
The LiDAR Driver is a program that hosts a server that will stream odometry data to a client that makes a connection. Unlike the LiDAR Driver, the MBot Driver receives goal poses from its client and uses a motion controller to drive to the set points.
